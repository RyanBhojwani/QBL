...
#    board = _annotate_and_filter(board)
    ...
    preview_cols = ["sport", "team", "market", "point",
                    "best_book", "sharp_p", "best_ip", "ev", "kelly"]
...
def build_edge_output(board: pd.DataFrame) -> pd.DataFrame:
    ...
    best_book, odds_from_best_book, sharp_odds, ev, kelly, sharpe
    (in that exact order).
    """
    df = board.copy()
    ...
    # Grab the odds that correspond to the bookmaker in `best_book`.
    # Pandas ≥2.0 removed DataFrame.lookup, so do it row-wise:
    #df["odds_from_best_book"] = df.apply(
    #    lambda r: r.get(r["best_book"], pd.NA), axis=1
    #).astype(float) 
    
    
    # Use only the first book before any comma in best_book for the lookup
    df["odds_from_best_book"] = pd.to_numeric(
        df.apply(
            lambda r: r.get(
                (str(r["best_book"]).split(",")[0].strip()) if pd.notna(r["best_book"]) else pd.NA,
                pd.NA,
                ),
            axis=1,
            ),
    ...
    df["implied_probability"] = df["best_ip"]  # same as training
