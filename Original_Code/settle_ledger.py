...
def main():
    ...
    # ── Write results ────────────────────────────────────────────────────────
    if sb_rows is not None:
        sb.upsert_settled_picks(graded_rows)   # copy API-graded rows to settled_picks
        sb.post_settlement_cleanup()            # copy manual W/L grades + delete all settled from tracked_picks
    else:
        if WRITE_BACK:
            ledger_updated.to_csv(LEDGER_PATH, index=False)
